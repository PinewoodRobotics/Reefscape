...
public class Swerve extends SubsystemBase {
    ...
    this.swerve =
      new SwerveDrive(
        new Config(
          new Communicator(),
          new Wheel[] {
            new Wheel(new Vec2(0.238125, 0.238125), m_rearLeftSwerveModule),
            new Wheel(new Vec2(0.238125, -0.238125), m_rearRightSwerveModule),
            new Wheel(new Vec2(-0.238125, -0.238125), m_frontRightSwerveModule),
            new Wheel(new Vec2(-0.238125, 0.238125), m_frontLeftSwerveModule),
          }
        )
      );
  }
  ...
  public void drive(Vec2 vector, double rotation, double speed) {
    swerve.drive(
      vector,
      Math.toRadians(CustomMath.wrapTo180(m_gyro.getAngle())),
      rotation,
      speed
    );
  }
